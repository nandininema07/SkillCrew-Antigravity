...
export function ArchieGamifiedRoadmap({
  ...
  /** Current profile XP (for capstone unlock). */
  accountXp?: number
  /** ISO timestamp when capstone was unlocked for this roadmap, if any. */
  capstoneUnlockedAt?: string | null
  /** Learning direction from the roadmap row (personalizes capstone copy + links). */
          ...
          <div className="mb-5 rounded-2xl border border-border/80 bg-muted/20 p-4 sm:p-5">
            ...
            {pipAnalysis.results_preview.length > 0 ? (
              <div className="mt-4 space-y-2">
                <p className="text-xs font-medium uppercase tracking-wide text-muted-foreground">Question notes</p>
                <div className="max-h-40 overflow-y-auto overscroll-contain rounded-lg border border-border/60 bg-card/80 [scrollbar-gutter:stable]">
                  <ul className="space-y-2.5 p-2.5 text-sm">
                    {pipAnalysis.results_preview.map((r, idx) => (
                      <li
                        key={`${r.question_id || idx}`}
                        className="min-w-0 break-words rounded-md border border-border/50 bg-muted/30 px-2.5 py-2.5"
                      >
                        <div className="flex flex-wrap items-baseline gap-x-2 gap-y-1">
                          ...
                          {r.topic ? (
                            <span className="min-w-0 flex-1 text-muted-foreground">{r.topic}</span>
                          ) : null}
                        </div>
                        {r.note ? (
